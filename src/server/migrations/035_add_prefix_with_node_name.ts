...
export const migration = {
  up: (db: Database.Database): void => {
    logger.debug('Running migration 035: Add prefix_with_node_name to notification preferences...');

    // Add prefix_with_node_name column with default value of 0 (false)
    db.exec(`
      ALTER TABLE user_notification_preferences
      ADD COLUMN prefix_with_node_name BOOLEAN DEFAULT 0
    `);
    logger.debug('Added prefix_with_node_name column');

    logger.debug('Migration 035 completed successfully');
  },

  down: (db: Database.Database): void => {
    logger.debug('Reverting migration 035: Remove prefix_with_node_name column...');
    ...
    db.exec(`
      ...
      SELECT id, user_id, enable_web_push, enable_apprise, enabled_channels,
             ...
             monitored_nodes, whitelist, blacklist, created_at, updated_at
    ...
    logger.debug('Migration 035 reverted');
  }
};
